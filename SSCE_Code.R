...
SSCE_EM_lambda2 = function(Y, X, num_update = 100, niter = 100, group_size, sigma2,a=1, b=1,
  ...
  for (update in 1:num_update) {
    ...
    for (i in 1:ngroup) {
      ...
      for(i in 1:ngroup)
      {
        ...
        l[i] = pi/(pi + (1 - pi)*(tau2[i]*tau2[i])^(-1/2)*
			(Sigma*Sigma)^(1/2)*exp(
    ...
    sigma2 = rinvgamma(1, shape = n/4 + sum(Z)/2 + alpha, 
		scale = (t(Y[1:(n/2)])%*%Y[1:(n/2)] - 2 * t(c(beta_vec[seq(1,p,2)], out.int, trt.eff)) %*% t(cbind(X[1:(n/2),seq(1,p,2)], 1, A))%*%Y[1:(n/2)] + t(c(beta_vec[seq(1,p,2)], out.int, trt.eff)) %*% 
...
SSCE <- function (Y, X, niter = 5100, burnin = 100, group_size, sigma2, a = 1, 
    b = 1, num_update = 100, niter.update = 100, verbose = FALSE, 
    alpha = 0.1, gamma = 0.1, pi_prior = TRUE, pi = 0.5, update_tau = TRUE, 
    option.weight.group = FALSE, option.update = "global", lambda2_update = NULL) 
{
    n = length(Y)
    p = dim(X)[2]
    ngroup = length(group_size)
    ...
    if (update_tau == TRUE){
        fit_for_lambda2 = SSCE_EM_lambda2(Y, X, num_update = num_update, 
    ...
    for (i in 1:ngroup) {
		...
		sigma2 = rinvgamma(1, shape = n/4 + sum(Z)/2 + alpha, 
		scale = (t(Y[1:(n/2)])%*%Y[1:(n/2)] - 2 * t(c(beta_vec[seq(1,p,2)], out.int, trt.eff)) %*% 
            t(cbind(X[1:(n/2),seq(1,p,2)], 1, A))%*%Y[1:(n/2)] + t(c(beta_vec[seq(1,p,2)], out.int, trt.eff)) %*% 
            t(cbind(X[1:(n/2),seq(1,p,2)], 1, A))%*%cbind(X[1:(n/2),seq(1,p,2)], 1, A) %*% c(beta_vec[seq(1,p,2)], 
            out.int, trt.eff) + s)/2 + gamma)
            
   
        if (pi_prior == TRUE) 
            pi = rbeta(1, shape1 = a + ngroup - sum(Z), shape2 = b + 
                sum(Z))
    }
    pos_mean = apply(coef, 1, mean)
    pos_median = apply(coef, 1, median)
